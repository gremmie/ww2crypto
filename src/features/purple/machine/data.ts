/**
 * Wiring data for the switches used in the PURPLE cipher machine.
 *
 ...
 * kept it 1-based. At runtime the data is converted to be 0-based.
 *
 * This data was obtained by running an OCR program on a PDF of the above paper.
 */

const SIXES_DATA: number[][] = [
  [2, 1, 3, 5, 4, 6],
  [6, 3, 5, 2, 1, 4],
  [1, 5, 4, 6, 2, 3],
  [4, 3, 2, 1, 6, 5],
...
// Convert to 0-based.
for (const table of DECRYPT_DATA) {
  for (const row of table) {
    for (const [k, val] of row.entries()) {
      row[k] = val - 1;
    }
  }
}

const invertTable = (table: number[][]): number[][] =>
  table.map((row) => row.map((_, j) => row.indexOf(j)));

export const ENCRYPT_DATA = DECRYPT_DATA.map(invertTable);
